...
static const NSInteger STATIC_DefaultColumnCpunt = 3;

@interface ZKCustomLayout ()
//存放attribute的数组
...
- (void)prepareLayout {
    ...
    self.var_sectionInsets = UIEdgeInsetsZero;
    ...
    for (NSInteger i = 0; i < sectionCount; i ++) {
        ...
        if ([_delegate respondsToSelector:@selector(collectionView:layout:columnNumberAtSection:)]) {
            self.var_columnCount = [_delegate collectionView:self.collectionView layout:self columnNumberAtSection:indexPat.section];
        }
        
        if ([_delegate respondsToSelector:@selector(collectionView:layout:insetForSectionAtIndex:)]) {
            self.var_sectionInsets = [_delegate collectionView:self.collectionView layout:self insetForSectionAtIndex:indexPat.section];
        }
        ...
        //2 初始化区的 y 值
        for (NSInteger i = 0; i < self.var_columnCount; i ++) {
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([_delegate respondsToSelector:@selector(collectionView:layout:columnNumberAtSection:)]) {
        self.var_columnCount = [_delegate collectionView:self.collectionView layout:self columnNumberAtSection:indexPath.section];
    }
    
    if ([_delegate respondsToSelector:@selector(collectionView:layout:andLineSpacingForSectionAtIndex:)]) {
        self.lineSpacing = [_delegate collectionView:self.collectionView layout:self andLineSpacingForSectionAtIndex:indexPath.section];
    }
    ...
    CGFloat cellWeight = (collectionViewWeight - self.var_sectionInsets.left - self.var_sectionInsets.right - (self.var_columnCount - 1) * self.interitemSpacing) / self.var_columnCount;
    
    CGFloat var_cellHeight = [self.delegate collectionView:self.collectionView layout:self heightForRowAtIndexPath:indexPath itemWidth:cellWeight];
    ...
    CGFloat cellX = self.var_sectionInsets.left + tempMinColumn * (cellWeight + self.interitemSpacing);
    ...
    attributes.frame = CGRectMake(cellX, cellY, cellWeight, var_cellHeight);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)elementKind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([elementKind isEqualToString:UICollectionElementKindSectionHeader]) {
        ...
        self.contentHeight += self.var_sectionInsets.top;
        ...
        self.contentHeight += self.var_sectionInsets.bottom;
...
- (NSInteger)var_columnCount {
    if (_var_columnCount) {
        return _var_columnCount;
    }  else {}
    return STATIC_DefaultColumnCpunt;
}
...
-(NSMutableArray *)columnHeights {
    if (!_columnHeights) {
        _columnHeights = [NSMutableArray arrayWithCapacity:STATIC_DefaultColumnCpunt];
